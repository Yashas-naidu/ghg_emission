...
with tab1:
    ...
    if st.button("Calculate Mobile Emissions"):
        ...
        try:
            ...
            description = df_res["description"].iloc[0]
            st.subheader("Description")
            st.write(description)
            gas_columns = ["CH4", "CO2e", "N2O", "biogenicCO2", "fossilFuelCO2"]
            gas_values = {gas: df_res[gas].iloc[0] for gas in gas_columns}
            non_zero_gases = {k: v for k, v in gas_values.items() if v > 0}
            zero_gases = {k: v for k, v in gas_values.items() if v == 0}
            if non_zero_gases:
                df_plot = pd.DataFrame(list(non_zero_gases.items()), columns=["Gas", "Emission (Metric Tonne)"])
                st.subheader("Emission Breakdown")
                fig_bar = px.bar(df_plot, x="Gas", y="Emission (Metric Tonne)", 
                         text="Emission (Metric Tonne)", color="Gas",
                         title="Carbon Emissions by Gas Type")
                fig_bar.update_traces(textposition='outside')
                st.plotly_chart(fig_bar)
                fig_pie = px.pie(df_plot, names="Gas", values="Emission (Metric Tonne)", 
                         title="Emission Distribution", hole=0.3)
                st.plotly_chart(fig_pie)
            
            if zero_gases:
                 st.subheader("Gases with Zero Emission")
                 for gas, value in zero_gases.items():
                      fig_gauge = go.Figure(go.Indicator(
                           mode="gauge+number",
                value=value,
                domain={'x': [0, 1], 'y': [0, 1]},
                title={'text': f"{gas} Emission"},
                gauge={
                    'axis': {'range': [0, 1]},  
                    'bar': {'color': "red"},  # Red bar to indicate zero emission
                    'bgcolor': "lightgray",  # Background fill
                    'borderwidth': 2,
                    'bordercolor': "black"
                }
            ))
...
with tab3:
    ...
    if st.button("Calculate Carbon Footprint"):
        ...
        # plotting
            gases = ["CH4", "CO2", "CO2e", "HFC", "N2O", "NF3", "PFC", "SF6"]
            gas_values = {gas: df_res[gas].iloc[0] for gas in gases}
            description = df_res["description"].iloc[0] 
            st.subheader("Description")
            st.write(description)
            def plot_gauge_chart(gas, value):
               fig = go.Figure(go.Indicator(
            mode="gauge+number",
            value=value,
            title={'text': f"{gas} Emission (Metric Tonne)"},
            gauge={
                'axis': {'range': [0, max(100, value * 1.2)]},  # Dynamic range
                'bar': {'color': "green"},
                'steps': [
                    {'range': [0, value * 0.5], 'color': "lightgreen"},
                    {'range': [value * 0.5, value * 0.8], 'color': "yellow"},
                    {'range': [value * 0.8, max(100, value * 1.2)], 'color': "red"},
                ],
            }
        ))
               st.plotly_chart(fig)
            for gas, value in gas_values.items():
                     st.subheader(f"{gas} Emission")
                     plot_gauge_chart(gas, value)
...
with tab4:
    ...
    if st.button("Calculate Carbon Emission"):
        ...
        # Convert to DataFrame for display
           df_res = pd.json_normalize(result)
           co2e_value = float(df_res["CO2e (metric tonne)"][0])
           description = df_res["description"][0]
           st.subheader("Description")
           st.write(description)
           st.subheader("CO2e Emissions Visualization")

           fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=co2e_value,
        title={"text": "CO2e (Metric Tonne)"},
        gauge={
            "axis": {"range": [0, 1000]},  # Adjust range as needed
            "bar": {"color": "green"},
            "steps": [
...
# Submit button
    if st.button("Calculate market emission"):
        ...
        try:
            ...
            co2e_value = float(market_df_res["CO2e (metric tonne)"][0])
            description = market_df_res["description"][0]
            st.subheader("Description")
            st.write(description)
            fig = go.Figure(go.Indicator(
        mode="gauge+number",
    ...
    if st.button("Submit Request"):
        ...
        try:
            ...
            try:
                ...
                st.subheader("Description")
                st.write(description)
                st.subheader("Gauge Plots for Emissions")
                for gas, value in gas_values.items():
                    fig_gauge = go.Figure(go.Indicator(
        mode="gauge+number",
        value=value,
        ...
        title={'text': f"{gas} Emission"},
        gauge={
            'axis': {'range': [0, max(gas_values.values())]},  
            'bar': {'color': "red" if value == 0 else "blue"},  
            'bgcolor': "lightgray" if value == 0 else "white",  
            'borderwidth': 2,
            'bordercolor': "black"
        }
    ))
                st.plotly_chart(fig_gauge)
                st.subheader("Comparative Bar Chart of Emissions")
                df_plot = pd.DataFrame(list(gas_values.items()), columns=["Gas", "Emission (Metric Tonne)"])
                fig_bar = px.bar(df_plot, x="Gas", y="Emission (Metric Tonne)", text="Emission (Metric Tonne)",
                 title="Comparison of Different Gas Emissions", color="Gas")
                fig_bar.update_traces(textposition="outside")
                st.plotly_chart(fig_bar)
